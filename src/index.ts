...
function emitDomWorker(
  ...
  tsWorkerOutput: string,
  forceKnownWorkerTypes: Set<string>
) {
...
function emitDomWeb(
  webidl: Browser.WebIdl,
  tsWebOutput: string,
  forceKnownWindowTypes: Set<string>
) {
...
function emitES6DomIterators(
  webidl: Browser.WebIdl,
  tsWebIteratorsOutput: string
) {
  fs.writeFileSync(
    tsWebIteratorsOutput,
    emitWebIDl(webidl, Flavor.ES6Iterators)
  );
}
...
function emitDom() {
  ...
  const tsWebIteratorsOutput = path.join(
    outputFolder,
    "dom.iterable.generated.js"
  );
